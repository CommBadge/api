// Package admin implements the admin-only management endpoints.
package admin
...
// DegradedHalter is the subset of the degraded-detector surface the
// maintenance endpoint needs. It is left nil in the production wiring so that
// enabling maintenance here never fights the app-level detector; tests inject
// a stub to assert the call.
...
func (h *AdminHandler) DisbandCommunity(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.CommunityStore.Delete(r.Context(), communityID); err != nil {
		common.RespondError(w, http.StatusInternalServerError, "failed to disband community")
		return
	}
	common.RespondJSON(w, http.StatusOK, map[string]string{"message": "disbanded"})
}
...
func (h *AdminHandler) SetMaintenance(w http.ResponseWriter, r *http.Request) {
	...
	common.RespondJSON(w, http.StatusOK, map[string]bool{"maintenance": body.Enabled})
}
...
func (h *AdminHandler) ReplyTicket(w http.ResponseWriter, r *http.Request) {
	...
	if err := common.DecodeJSON(r, &body); err != nil || body.Body == "" {
...
func (h *AdminHandler) TicketCount(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		common.RespondError(w, http.StatusInternalServerError, "failed to count tickets")
		return
	}
	common.RespondJSON(w, http.StatusOK, map[string]int{"count": count})
}
...
func (h *AdminHandler) UpdateTicketStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil || body.Status == "" {
		common.RespondError(w, http.StatusBadRequest, "status is required")
		return
	}

	if err := h.TicketStore.AdminUpdateStatus(r.Context(), ticketID, body.Status); err != nil {
		common.RespondError(w, http.StatusInternalServerError, "failed to update status")
		return
	}
	common.RespondJSON(w, http.StatusOK, map[string]string{"message": "status updated"})
}
...
func (h *AdminHandler) GetUserWarnings(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		common.RespondError(w, http.StatusInternalServerError, "failed to get warnings")
		return
	}
	common.RespondJSON(w, http.StatusOK, warnings)
}
...
func (h *AdminHandler) WarnUser(w http.ResponseWriter, r *http.Request) {
	...
	if err := common.DecodeJSON(r, &body); err != nil || body.Reason == "" {
		common.RespondError(w, http.StatusBadRequest, "reason is required")
		return
	}

	if err := h.UserStore.AddWarning(r.Context(), userID, adminID, body.Reason); err != nil {
		common.RespondError(w, http.StatusInternalServerError, "failed to add warning")
		return
	}
	common.RespondJSON(w, http.StatusOK, map[string]string{"message": "warning added"})
}
...
func (h *AdminHandler) BanUser(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.UserStore.BanUser(r.Context(), userID, body.Reason); err != nil {
		common.RespondError(w, http.StatusInternalServerError, "failed to ban user")
		return
	}
	common.RespondJSON(w, http.StatusOK, map[string]string{"message": "user banned"})
}
...
func (h *AdminHandler) UnbanUser(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.UserStore.UnbanUser(r.Context(), userID); err != nil {
		common.RespondError(w, http.StatusInternalServerError, "failed to unban user")
		return
	}
	common.RespondJSON(w, http.StatusOK, map[string]string{"message": "user unbanned"})
}